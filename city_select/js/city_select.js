const DEF_SITE_DATA = { province: "辽宁省",city:"锦州市",area:"北镇市"};
const SITE_DATA = {
  ...
  "0_7_12": ["太和区", "古塔区", "凌河区", "凌海市", "北镇市", "黑山县", "黑山镇", "义县", "义州镇"],
...
var Box = React.createClass({
  ...
  getDefaultProps: function() {//默认值没用一直传了
    return {
      valueIndex: 0
    };
  },
  ...
  inputOnChange: function(e){
    ...
    if(complete){
      this.props.topHandle();
    }
  },
  render: function(){
    var rows = [];
    this.props.valueArr.forEach((v, i) => {
      rows.push(<Options level={this.props.placeholder} topHandle={this.props.topHandle} key={i} name={v} />);
    });
    var value = this.props.valueArr[this.props.valueIndex];
    return (
      <div className='box'>
        <input
          type='text'
...
var Wrap = React.createClass({
  getInitialState: function(){
    var provinceList = this.props.siteData['0'];
    var provinceIndex = provinceList.indexOf(this.props.default.province);
    var cityList = this.props.siteData['0_' + provinceIndex];
    var cityIndex = cityList.indexOf(this.props.default.city);
    var areaList = this.props.siteData['0_' + provinceIndex + '_' + cityIndex];
    var areaIndex = areaList.indexOf(this.props.default.area);
    return {
  ...
  topHandle: function(value, level){
    //联动
    if(level == '省份'){
      var provinceIndex = this.state.provinceList.indexOf(value);
      this.setState({
        provinceIndex: provinceIndex,
        cityList: this.props.siteData['0_' + provinceIndex],
        cityIndex: 0,
        areaList: this.props.siteData['0_' + provinceIndex + '_0'],
        areaIndex: 0
      });
    }else if(level == '城市'){
      var cityIndex = this.state.cityList.indexOf(value);
      this.setState({
        cityIndex: cityIndex,
        areaList: this.props.siteData['0_' + this.state.provinceIndex + '_' + cityIndex],
        areaIndex: 0
      });
    }else if(level == '地区'){
      this.setState({
        areaIndex: this.state.areaList.indexOf(value)
      });
    }
  },
  render: function(){
    return (
      <div>
        <Box
          valueArr={this.state.provinceList}
          valueIndex={this.state.provinceIndex}
          topHandle={this.topHandle}
          placeholder={this.state.provincePlaceholder} />
        <Box
          valueArr={this.state.cityList}
          valueIndex={this.state.cityIndex}
          cvalue={this.state.city}
          topHandle={this.topHandle}
          placeholder={this.state.cityPlaceholder} />
        <Box
          valueArr={this.state.areaList}
          valueIndex={this.state.areaIndex}
          topHandle={this.topHandle}
          placeholder={this.state.areaPlaceholder} />
      </div>
    );
  }
});
